...
export async function runModelCouncil(input: RunModelCouncilInput): Promise<ModelCouncilResult> {
  ...
  let ratifications = hasCandidate
    ? await ratifyCandidate(members, prompt, rounds, candidateConsensus, input.evidencePack)
    : [];
  ...
  if (shouldAttemptRepair(ratifications)) {
    ...
    if (revisedCandidate.length > 0 && candidateChanged(candidateConsensus, revisedCandidate)) {
      ...
      ratifications = await ratifyCandidate(members, prompt, rounds, revisedCandidate, input.evidencePack);
  ...
  if (isRatifiedWithEdits(ratifications)) {
    ...
    try {
      const foldedText =
        parseCandidateConsensus(
          await askMember(chair, buildSynthesisMessages(prompt, candidateConsensus, ratifications, chair)),
        ) ?? "";
...
async function askDeliberation(
  ...
  const content = await askMember(member, messages);
  ...
  const resendContent = await askMember(member, appendProtocolResendNote(messages));
