...
| WU-B8 | docs | done | WU-B1, WU-B2, WU-B3, WU-B4, WU-B5, WU-B6, WU-B7 |

Counts: done = 9, ready = 0, blocked = 0, deferred = 0 (Wave C recorded as deferred work, not decomposed into units).
...
# handoff WU-B8: done @ 2026-06-09 — docs-only unit. docs/council.md extended with a
#   "Structured deliberation subsystem (Claim-Ledger Delphi)" section documenting every shipped
#   behavior: AGENTS_COUNCIL_STRUCTURED (default OFF, opt-in, flag-off byte-for-byte legacy);
#   typed Claim schema + provenance enum (repo_fact|source_claim|assumption) + Zod-parse-fail
#   text fallback (B1); the pure Level-1 SOURCE_ID_MISMATCH / UNLABELED_CLAIM /
#   ASSUMPTION_NO_VERIFICATION preconditions raising an absolute FACTUAL_ERROR block, with the
#   Wave-B heuristic-boundary FYIs (unlabeled unreachable via applier; assumption always blocks —
#   no cheapest_verification field) (B2); optional evidencePack prepended to round-0 proposals,
#   byte-identical legacy when absent (B3); minorityReport[] on the result + `## Minority Report`
#   MD section emitted on `blocked`, CLI non-zero exit on blocked (B4); trace-{ts}.json
#   (council_trace.v1) consumer contract + equal member weights==1 (B5); issue-map-{ts}.json
#   (issue_map.v1) deterministic exact-match agreed/contested partition that controls nothing,
#   INV-3 (B6); DEFAULT_MEMBER_IDS=[claude,chatgpt,gemini] odd-roster default + AGENTS_COUNCIL_MEMBERS
#   override / n=2 draft (B7); enum-freeze INV-6 (not_attempted|ratified|blocked; qualified_consensus
#   deferred/nested-only); the new env-var table; and a Wave-C entry-gate cross-link (parse-fail<5%
#   over >=50 runs AND beats self-consistency). NO source/test changed; no MDOC stub created (nested
#   repo has no MDOC registry). gate_mdoc_clean -> done. format:check unchanged (81 files, same 3
#   pre-existing reds: harness/run_tier5_scheduling_council.ts + harness/model_council_result_r5.json
#   + council_runs/*.json, all byte-identical to HEAD; council.md is Markdown and not checked by
#   biome). All 9 units done. gate_regression stays planned (acceptance-owned). See
#   _reports/WU-B8-execution.md.
